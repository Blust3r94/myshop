...
1. **Pagina prodotto** (`src/app/prodotti/[slug]/page.tsx`) — galleria immagini, scelta taglia/colore, bottone "aggiungi al carrello" che usa `useCartStore`
2. **Pagina carrello** (`src/app/carrello/page.tsx`) — riepilogo, modifica quantità, bottone checkout che chiama `/api/checkout`
3. **Pagina conferma ordine** (`src/app/checkout/successo/page.tsx`)
4. **Pannello admin minimale** — form per aggiungere/modificare prodotti e varianti (protetto da `ADMIN_PASSWORD`, non serve auth completa per partire)
5. **Configurazione webhook Stripe in locale** — `stripe listen --forward-to localhost:3000/api/webhook` (Stripe CLI)
